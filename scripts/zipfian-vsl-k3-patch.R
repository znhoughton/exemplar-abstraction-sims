# Patch run: adds k=3 results to grid_results_model3.csv.
# Run once from the simulations/ root. Appends to the existing CSV.
...
N_OBS_GRID     <- unique(round(exp(seq(log(N_VERBS), log(MAX_NOBS_TOTAL), length.out = 20L))))
P_THRESH       <- 0.001
CLASS_FRAC     <- 0.10
DJS_THRESH     <- 0.01
SUSTAIN        <- 3L
...
run_one <- function(mu, sigma, n_preferred, item_overlap, class_overlap, add_k, zipf_s, seed) {
  set.seed(seed)
  ...
  zipf_probs <- (1.0 / ranks^zipf_s) / sum(1.0 / ranks^zipf_s)
  ...
  for (v in seq_len(N_VERBS)) {
    ...
    verb_streams[[v]] <- if (length(idx) > 0L)
      sample(VOCAB_SIZE, length(idx), replace = TRUE, prob = P_true[v, ])
    else integer(0L)
  }
  ...
  ob_nobs <- NA_integer_; ow_nobs <- NA_integer_
  ow_streak <- 0L; ow_start <- NA_integer_
  for (ci in seq_along(N_OBS_GRID)) {
    n_total <- N_OBS_GRID[ci]
    P_hat <- matrix(0.0, N_VERBS, VOCAB_SIZE)
    for (v in seq_len(N_VERBS)) {
      n_v    <- obs_at_chk[v, ci]
      counts <- if (n_v > 0L) tabulate(verb_streams[[v]][seq_len(n_v)], nbins = VOCAB_SIZE)
                else integer(VOCAB_SIZE)
    ...
    if (is.na(ow_nobs)) {
      ...
      } else { ow_streak <- 0L; ow_start <- NA_integer_ }
    }
...
run_combo <- function(row_i) {
  ...
  data.frame(
    ...
    item_overlap  = p$item_overlap, class_overlap = p$class_overlap,
    add_k         = p$add_k, zipf_s = p$zipf_s, n_preferred = N_PREFERRED,
